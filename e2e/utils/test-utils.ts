...
export async function click(
  page: Page,
  opts: { xpath?: string; css?: string }
): Promise<void> {
  const { xpath, css } = opts;
  if (xpath) {
    return page.evaluate((selector) => {
      const node: HTMLElement = document.evaluate(
        selector,
        document,
        null,
  ...
  if (css) {
    return page.evaluate((selector) => {
      const node: HTMLElement = document.querySelector(selector) as HTMLElement;
      node.click();
    }, css);
  }
}
...
export async function clickRecaptcha(page: Page): Promise<void> {
...
export async function newUserRegistrationSelfBypass(page: Page): Promise<void> {
  ...
  const selfBypass = await page.waitForXPath(
    `${selfBypassXpath}//div[@role="button"]`,
    { visible: true }
  );
  ...
  await waitForText(
    page,
    'Bypass action is complete. Reload the page to continue.',
...
export async function performActions(
  page: Page,
  fields: {
    id: { textOption: XPathOptions; affiliated?: ElementType };
    value?: string;
    selected?: boolean;
  }[]
): Promise<void> {
...
export async function performAction(
  ...
  switch (identifier.textOption.type.toLowerCase()) {
    case 'radio':
      const radioELement = await RadioButton.findByName(
        page,
        identifier.textOption
      );
      await radioELement.select();
      break;
    case 'checkbox':
      const checkboxElement = await Checkbox.findByName(
        page,
        identifier.textOption
      );
      ...
      break;
    case 'text':
      const textboxElement = await Textbox.findByName(
        page,
        identifier.textOption
      );
      ...
      break;
    case 'textarea':
      const textareaElement = await Textarea.findByName(
        page,
        identifier.textOption
      );
...
export async function createWorkspace(
  ...
  console.log(
    `Created workspace "${name}" with CDR Version "${cdrVersionName}"`
  );
  await workspacesPage.load();
...
export async function findOrCreateWorkspace(
  ...
  const existingWorkspaces = await workspaceCard.getWorkspaceMatchAccessLevel(
...
export async function centerPoint(
