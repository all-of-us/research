...
export default class WorkspaceAboutPage extends WorkspaceBase {
  ...
  async findUserInCollaboratorList(
    username: string
  ...
  async getCreationDate(): Promise<string> {
    ...
    const creationDate = await this.page.waitForXPath(creationDateXpath, {
      visible: true,
    });
  ...
  async getLastUpdatedDate(): Promise<string> {
    ...
    const lastUpdatedDate = await this.page.waitForXPath(lastUpdatedDateXpath, {
      visible: true,
    });
