...
export default class CohortSearchPage extends AuthenticatedPage {
  ...
  async isLoaded(): Promise<boolean> {
    await Promise.all([
      this.page.waitForXPath('//*[@id="cohort-search-container"]', {
        visible: true,
      }),
      this.page.waitForXPath('//*[@role="button"]/img[@alt="Go back"]', {
        visible: true,
      }),
    ]);
  ...
  async waitForEthnicityCriteriaLink(
  ...
  async addAge(minAge: number, maxAge: number): Promise<string> {
    ...
    const count = await waitForNumericalString(
      this.page,
      `${this.containerXpath}//*[@id="age-count"]`
    );

    // Click ADD SELECTION to add selected age range
    await Button.findByName(this.page, {
      name: LinkText.AddSelection,
  ...
  async drageAgeSlider(): Promise<void> {
    ...
    const lowerNumberInputHandle = await this.page.waitForXPath(
      getXpath('noUi-handle-lower'),
      { visible: true }
    );
    const upperNumberInputHandle = await this.page.waitForXPath(
      getXpath('noUi-handle-upper'),
      { visible: true }
    );
