...
export default class GoogleLoginPage {
  ...
  async email(): Promise<ElementHandle> {
    return this.page.waitForXPath(FieldSelector.EmailInput, {
      visible: true,
      timeout: 60000,
    });
  }
  ...
  async password(): Promise<ElementHandle> {
    return this.page.waitForXPath(FieldSelector.PasswordInput, {
      visible: true,
      timeout: 60000,
    });
  }
  ...
  async loginButton(): Promise<ElementHandle> {
    return this.page.waitForXPath(FieldSelector.LoginButton, {
      visible: true,
      timeout: 60000,
    });
  }
  ...
  async cookiePolicyLink(): Promise<ElementHandle> {
    return this.page.waitForXPath(FieldSelector.CookiePolicyLink, {
      visible: true,
      timeout: 60000,
    });
  }
  ...
  async enterEmail(userEmail: string): Promise<void> {
    try {
      ...
      const useAnotherAccountXpath =
        '//*[@role="link"]//*[text()="Use another account"]';
      const elemt1 = await Promise.race([
        this.page.waitForXPath(FieldSelector.EmailInput, {
          visible: true,
          timeout: 60000,
        }),
        this.page.waitForXPath(useAnotherAccountXpath, {
          visible: true,
          timeout: 60000,
        }),
      ]);
      ...
      if (link) {
        const isLink = await this.page.evaluate(
          (e1, e2) => e1 === e2,
          elemt1,
          link
        );
  ...
  async submit(): Promise<void> {
    ...
    await this.page
      .waitForSelector('app-signed-in', { timeout: 60 * 1000 })
      .catch(async (err) => {
  ...
  async loginAs(email: string, paswd: string): Promise<void> {
    await this.login(email, paswd);
  }

  async clickCreateAccountButton(): Promise<void> {
    const button = await Button.findByName(this.page, {
      name: 'Create Account',
    });
  ...
  private async fillOutRecoverEmail(): Promise<boolean> {
    const recoverEmailXpath =
      '//input[@type="email" and @aria-label="Enter recovery email address"]';
