...
export default class CohortParticipantDetailPage extends AuthenticatedPage {
  ...
  async getParticipantIDnum(): Promise<string> {
    const selector =
      '//*[@class="detail-header"]//span[contains(., "Participant ")]';
  ...
  async clickPenIconHelpSideBar(): Promise<void> {
    const clickPenIcon = await this.page.waitForXPath(
      '//*[@data-test-id="help-sidebar-icon-annotations"]',
      { visible: true }
    );
    await clickPenIcon.click();
    await this.page.waitForXPath('//*[@data-test-id="sidebar-content"]', {
      visible: true,
    });
