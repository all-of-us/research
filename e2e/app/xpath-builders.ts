...
export function buildXPath(xOpts: XPathOptions, container?: Container): string {
  ...
  if (type === 'icon') {
    if (iconShape === undefined) {
      throw new Error(
        'Incorrect XPathOptions configuration for Icon: set "iconShape" value.'
      );
    }
  }
  ...
  switch (type) {
    ...
    case ElementType.Icon: // clickable icon
      const tag =
        iconShape === undefined ? '*' : `clr-icon[@shape="${iconShape}"]`;
