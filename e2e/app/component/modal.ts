...
export default class Modal extends Container {
  ...
  async getTextContent(): Promise<string[]> {
    ...
    return fp.flow(
      fp.map(async (elem: ElementHandle) =>
        (await getPropValue<string>(elem, 'textContent')).trim()
      ),
  ...
  async clickButton(
    ...
    waitOptions: { waitForNav?: boolean; waitForClose?: boolean } = {}
  ): Promise<void> {
    ...
    await Promise.all(
      fp.flow(
        fp.filter<{ shouldWait: boolean; waitFn: () => Promise<void> }>(
          'shouldWait'
        ),
