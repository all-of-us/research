...
export default class CopyModal extends Modal {
   ...
   */
  async beginCopyToAnotherWorkspace(
  ...
  async waitForSelectOption(workspaceName?: string): Promise<ElementHandle> {
    const xpathSubstr =
      'starts-with(@id, "react-select-") and contains(@id, "-option-")';
    if (workspaceName === undefined) {
      // Without a workspace name, select the second (or any) option in dropdown.
      return this.page.waitForXPath(
        `${this.getXpath()}//*[${xpathSubstr} and text()]`,
        { visible: true }
      );
    }
    return this.page.waitForXPath(
      `${this.getXpath()}//*[${xpathSubstr} and text()="${workspaceName}"]`,
      { visible: true }
    );
  }
}
