...
export default class Table extends Container {
  private trXpath: string = super.xpath + '//tbody/tr';
  private theadXpath: string = super.xpath + '/thead/tr/th';

  constructor(page: Page, xpath: string, container?: Container) {
    super(
      page,
      container === undefined ? xpath : `${container.getXpath()}${xpath}`
    );
  }

  async asElement(): Promise<ElementHandle> {
    return this.page
      .waitForXPath(this.xpath, { timeout: 1000, visible: true })
  ...
  async getColumnIndex(columName: string): Promise<number> {
    ...
    const handle = await this.page.waitForXPath(columnIndexXpath, {
      visible: true,
    });
