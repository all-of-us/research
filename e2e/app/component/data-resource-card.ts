...
export default class DataResourceCard extends CardBase {
  ...
  static async findAllCards(
    page: Page,
    timeOut = 2000
  ): Promise<DataResourceCard[]> {
    await waitWhileLoading(page);
    try {
      await page.waitForXPath(DataResourceCardSelector.cardRootXpath, {
        visible: true,
        timeout: timeOut,
      });
    } catch (e) {
    ...
    const resourceCards = cards.map((card) =>
      new DataResourceCard(page).asCard(card)
    );
  ...
  async getResourceName(): Promise<string> {
    const elemt = await this.cardElement.$x(
  ...
  async getCardType(): Promise<string> {
    const [element] = await this.cardElement.$x(
      DataResourceCardSelector.cardTypeXpath
    );
  ...
  async getLink(): Promise<ElementHandle> {
    const [element] = await this.cardElement.$x(
      this.resourceNameLinkSelector()
    );
    return element;
  }

  async getResourceCard(
    cardType: ResourceCard = ResourceCard.Cohort
  ): Promise<DataResourceCard[]> {
  ...
  async clickResourceName(): Promise<void> {
    const elemts = await this.asElementHandle().$x(
      `.//*[${DataResourceCardSelector.cardNameXpath}]`
    );
    await Promise.all([
  ...
  async cardExists(cardName: string, cardType: ResourceCard): Promise<boolean> {
    ...
    const names = await Promise.all(
      cards.map((item) => item.getResourceName())
    );
