...
export default class Navigation {
  ...
  static async navMenu(page: Page, destinationApp: NavLink): Promise<void> {
    const findMenuItem = async (): Promise<ElementHandle | null> => {
      ...
      const angleIconXpath = buildXPath({
      ...
      if (!applink) {
        ...
        if (
          destinationApp === NavLink.PROFILE ||
          destinationApp === NavLink.SIGN_OUT
        ) {
          // Open User submenu if needed
          if (!applink) {
            await username.click();
            return page.waitForXPath(appLinkXpath, {
              timeout: 2000,
              visible: true,
            });
          }
