...
async function copyNotebookTest(
  ...
  destCdrVersionName: string
) {
  const destWorkspace = await createWorkspace(
    page,
    destCdrVersionName
  ...
  await analysisPage.copyNotebookToWorkspace(
    sourceNotebookName,
    destWorkspace,
    copiedNotebookName
  );
  ...
  // Delete notebook
  const deleteModalTextContent = await analysisPage.deleteResource(
    sourceNotebookName,
    ResourceCard.Notebook
  );
  expect(deleteModalTextContent).toContain(
    `Are you sure you want to delete Notebook: ${sourceNotebookName}?`
  );

  // Perform actions in copied notebook.
  // Open destination Workspace
  await findOrCreateWorkspace(page, {
    workspaceName: destWorkspace,
  }).then((card) => card.clickWorkspaceName());
  ...
  // Delete notebook
  const modalTextContent = await analysisPage.deleteResource(
    copiedNotebookName,
    ResourceCard.Notebook
  );
...
async function createCustomCdrVersionWorkspace(
  cdrVersion: string
): Promise<string> {
...
describe('Workspace owner Jupyter notebook action tests', () => {
  ...
  test(
    ...
    async () => {
      defaultCdrWorkspace = await createCustomCdrVersionWorkspace(
