...
describe('Workspace owner Jupyter notebook action tests', () => {
  ...
  test(
    ...
    async () => {
      ...
      const workspaceAnalysisPage = await workspacesPage.createNotebook({
        workspaceName,
        notebookName,
      });
      ...
      await workspaceAnalysisPage
        .createNewNotebookLink()
      ...
      const errorExists = await page.waitForXPath(errorTextXpath, {
        visible: true,
      });
      ...
      await workspaceAnalysisPage.deleteResource(
        notebookName,
  ...
  test(
    ...
    async () => {
      ...
      const duplNotebookName = await workspaceAnalysisPage.duplicateNotebook(
        notebookName
      );
  ...
  test(
    ...
    async () => {
      ...
      let cardExists = await newNotebookCard.cardExists(
        newName,
        ResourceCard.Notebook
      );
      expect(cardExists).toBe(true);

      cardExists = await newNotebookCard.cardExists(
        notebookName,
        ResourceCard.Notebook
      );
