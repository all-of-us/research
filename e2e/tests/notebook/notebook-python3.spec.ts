...
describe('Jupyter notebook tests in Python language', () => {
  ...
  test(
    ...
    async () => {
      ...
      const cell2OutputText = await notebook.runCodeCell(2, {
      ...
      await notebook.runCodeCell(3, {
        codeFile: 'resources/python-code/simple-pyplot.py',
      });
      ...
      const codeSnippetOutput = await notebook.runCodeCell(4, {
        code: codeSnippet,
      });
      expect(codeSnippetOutput).toEqual(
        expect.stringContaining('/usr/local/share/jupyter/kernels/python3')
      );
