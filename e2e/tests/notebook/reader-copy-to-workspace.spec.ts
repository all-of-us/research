...
describe('Workspace reader Jupyter notebook action tests', () => {
  ...
  test(
    ...
    async () => {
      const workspaceName = await createWorkspace(page).then((card) =>
        card.clickWorkspaceName()
      );

      let dataPage = new WorkspaceDataPage(page);
      const notebookName = makeRandomName('py');
      const notebook = await dataPage.createNotebook(
        notebookName,
        Language.Python
      );
      ...
      await WorkspaceCard.findCard(page, workspaceName).then((card) =>
        card.clickWorkspaceName()
      );
      ...
      const newPage = await signInAs(
        config.collaboratorUsername,
        config.userPassword
      );

      // Create a new Workspace. This is the copy to workspace.
      const collaboratorWorkspaceName = await findOrCreateWorkspace(
        newPage
      ).then((card) => card.getWorkspaceName());

      // Verify shared Workspace Access Level is READER.
      const workspaceCard = await WorkspaceCard.findCard(
        newPage,
        workspaceName
      );
      ...
      let notebookCard = await dataResourceCard.findCard(
        notebookName,
      ...
      expect(
        await snowmanMenu.isOptionDisabled(Option.CopyToAnotherWorkspace)
      ).toBe(false);
      ...
      await newAnalysisPage.copyNotebookToWorkspace(
        notebookName,
      ...
      await modal.clickButton(LinkText.GoToCopiedNotebook, {
        waitForClose: true,
      });

      // Verify current workspace is collaborator Workspace.
      await newAnalysisPage.waitForLoad();
      const workspaceLink = await Link.findByName(newPage, {
      ...
      notebookCard = await dataResourceCard.findCard(
        copiedNotebookName,
        ResourceCard.Notebook
      );
      ...
      expect(await copyNotebookCardMenu.isOptionDisabled(Option.Rename)).toBe(
        false
      );
      expect(
        await copyNotebookCardMenu.isOptionDisabled(Option.Duplicate)
      ).toBe(false);
      expect(await copyNotebookCardMenu.isOptionDisabled(Option.Delete)).toBe(
        false
      );
      expect(
        await copyNotebookCardMenu.isOptionDisabled(
          Option.CopyToAnotherWorkspace
        )
      ).toBe(false);
      await notebookCard.clickSnowmanIcon(); // close menu

      await newAnalysisPage.deleteResource(
        copiedNotebookName,
        ResourceCard.Notebook
      );
    },
