...
describe('Cohorts Action tests', () => {
  const workspace = 'e2eCohortsActionTest';
  const cohortName = 'e2eCohortsActionTestCohortName';
  let cohortCard: DataResourceCard;

  beforeEach(async () => {
    await signInWithAccessToken(page);
    await findOrCreateWorkspace(page, { workspaceName: workspace });
    const dataPage = new WorkspaceDataPage(page);
    cohortCard = await dataPage.createCohort(cohortName);
  });

  test('Create Review Set for 100 participants', async () => {
    ...
    console.log(`Created Cohorts Review Set with ${reviewSetNumberOfParticipants} participants.`);
