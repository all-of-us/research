...
describe('Editing and rename Concept Set', () => {
  ...
  xtest('Workspace OWNER can edit Concept Set', async () => {
    ...
    expect(Number.isNaN(parseInt(row1.rollUpCount.replace(/,/g, ''), 10))).toBe(
      false
    );
    expect(Number.isNaN(parseInt(row2.rollUpCount.replace(/,/g, ''), 10))).toBe(
      false
    );
    ...
    console.log(
      `Added new Concept to existing Concept Set "${conceptSetName}"`
    );

    // Concept Set saved. Click Concept Set link. Land on Concept Set page.
    const conceptSetPage = await conceptSetActionsPage.openConceptSet(
      conceptSetName
    );
